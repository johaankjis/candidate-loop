#!/usr/bin/env python3
"""Run one live OpenRouter-backed CandidateLoop pass and validate its safe outcome.

This mirrors bedrock_smoke.py: the same Strands agent, tool adapter, rollback, and safety
expectations apply. Only the model provider differs.
"""
...
def main() -> None:
    ...
    if settings.model_provider != "openrouter":
        raise AgentConfigurationError(
            "OpenRouter smoke requires CANDIDATELOOP_MODEL_PROVIDER=openrouter; "
            f"observed '{settings.model_provider}'. Use scripts/bedrock_smoke.py for Bedrock."
        )
    ...
    if (
        result.execution_mode != "strands_openrouter"
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"OpenRouter smoke failed: {error}") from None
